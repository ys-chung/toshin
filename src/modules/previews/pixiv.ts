    ...
    for (const endpoint of endpoints) {
        try {
            const infoRes = await fetch(
                `${endpoint}/api/pixiv/illust?id=${illustId}`
            )
...
function generateReplyFromArtworkInfo(
    ...
    const nsfwWarning =
        artworkInfo.nsfw && !isMessageChannelAgeRestricted(message)
...
export async function pixivPassive(
    ...
    discordClient.on("messageCreate", async (message) => {
        if (
            ...
            const pixivMatches = [
                ...message.cleanContent.matchAll(messageArtworkIdRegex)
            ]
            ...
            if (pixivMatches.length === 1) {
                ...
                try {
                    ...
                    const reply = generateReplyFromArtworkInfo(
                        message,
                        artworkInfo
                    )
